...
static cecs_instance_group create_duck_instance(
    ...
    cecs_graphics_system *system,
    cecs_arena *builder_arena,
    cecs_texture_in_bank_range2_u8_attribute textures[4],
    cecs_texture_subrect2_f32_attribute subrects[4]
) {
...
int main(void) {
    ...
    const float camera_near =   0.45f;
    const float camera_far =    45.0f;
    static const double pi = 3.;
    const float camera_fov_y = (float)(pi * 0.5);
    cecs_camera_pack camera = (cecs_camera_pack){
        .bundle = (cecs_camera_bundle){
            .camera = cecs_camera_create_perspective(camera_fov_y, camera_far, camera_near),
            .position = (cecs_position3_f32){ .x = 0.0f, .y = 0.0f, .z = -camera_far * 0.5f },
    ...
    while (!glfwWindowShouldClose(window) && !render_error) {
        ...
        WGPUColor tint = { sin(t_sec) * 0.5f + 0.5f, cos(t_sec) * 0.5f + 0.5f, 0.0f, 1.0f };
