...
cecs_file_texture2_builder *cecs_file_texture2_builder_load_into(cecs_file_texture2_builder *builder, const char *path, const uint_fast8_t texture_slot) {
    ...
    assert(width == builder->builder.descriptor.descriptor.size.width && "error: width must match requirements");
    assert(height == builder->builder.descriptor.descriptor.size.height && "error: height must match requirements");
    assert(channels == builder->channel_count && "error unexpected: channel count mismatch");

    if (builder->builder.descriptor.flags & cecs_texture_builder_descriptor_config_generate_mipmaps) {
        builder->builder.descriptor.flags |= cecs_texture_builder_descriptor_config_alloc_mipmaps;
    }
    return cecs_texture_builder_take_into(builder, texture_data, texture_slot);
}
...
size_t cecs_file_texture2_builder_build_alloc(
    ...
    return cecs_mem_texture_builder_build_alloc(
        &builder->builder,
        context,
        view_descriptor,
