...
extern inline void *cecs_flatmap_bucket_get_value_mut_unchecked(const cecs_flatbucket_mut bucket, const uint_fast8_t index, const size_t value_size);
...
extern inline void *cecs_flatmap_bucket_get_value_mut(const cecs_flatbucket_mut bucket, const uint_fast8_t index, const size_t value_size);
...
extern inline cecs_flatmap_hash *cecs_flatmap_bucket_get_key_mut_unchecked(const cecs_flatbucket_mut bucket, const uint_fast8_t index);
extern inline const cecs_flatmap_hash *cecs_flatmap_bucket_get_key(const cecs_flatbucket bucket, const uint_fast8_t index);
extern inline cecs_flatmap_hash *cecs_flatmap_bucket_get_key_mut(const cecs_flatbucket_mut bucket, const uint_fast8_t index);
...
cecs_flatmap cecs_flatmap_create_with_capacity(cecs_allocator *allocator, const size_t bucket_count, const size_t value_size) {
    ...
    uint8_t *const allocation = (uint8_t *)cecs_allocator_alloc_aligned(
        allocator,
        cecs_flatmap_allocation_size(value_size, bucket_count),
        8
    );
    cecs_flatmap set = {
    ...
    for (size_t i = 0; i < bucket_count; i++) {
        cecs_flatbucket_mut bucket = cecs_flatmap_get_bucket_mut(&set, i, value_size);
...
void cecs_flatmap_destroy(cecs_flatmap *set, cecs_allocator *allocator, const size_t value_size) {
    if (set->allocation != NULL) {
        cecs_allocator_free(allocator, set->allocation, cecs_flatmap_allocation_size(value_size, set->bucket_count));
        set->allocation = NULL;
    }
    set->bucket_count = 0;
    set->values_count = 0;
...
cecs_flatbucket_mut cecs_flatmap_find_insert_bucket_expect(
    ...
    do {
        bucket_mut = cecs_flatmap_get_bucket_mut(set, next_bucket_index, value_size);
        bucket = cecs_flatbucket_from(bucket_mut);
        const uint_fast8_t index = cecs_flatbucket_find_hash_index(bucket, hash, hash7, sizeof(cecs_flatmap_hash));
...
void *cecs_flatmap_insert_into_bucket_expect(
    ...
    const uint_fast8_t index = cecs_flatbucket_get_count(cecs_flatbucket_from(bucket));
...
void *cecs_flatmap_insert_within_expect(
    ...
    const cecs_flatbucket_mut insert = cecs_flatmap_find_insert_bucket_expect(
        set,
        hash,
        value_size
    );
...
void *cecs_flatmap_find_or_insert(
    ...
    uint_fast8_t index;
    const cecs_flatbucket_mut insert = cecs_flatmap_find_insert_bucket(
        set,
        hash,
        value_size,
        &index
...
void cecs_flatmap_remove_from_bucket_stable_expect(
    ...
    cecs_flatbucket_remove_expect(bucket_mut, index, value_size);
    if (index < last_index) {
        *cecs_flatmap_bucket_get_key_mut(bucket_mut, index) = last_hash;
    }
    --set->values_count;
}
