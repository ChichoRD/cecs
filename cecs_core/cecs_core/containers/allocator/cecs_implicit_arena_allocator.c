...
static inline const uint8_t *cecs_implicit_arena_allocator_node_next_start(const cecs_implicit_arena_allocator_node node) {
    return (uint8_t *)node.next + sizeof(node) - node.next_size;
}
static inline const uint8_t *cecs_implicit_arena_allocator_node_next_end(const cecs_implicit_arena_allocator_node node) {
