...
static const cecs_flatset_hash_low_fast cecs_flatbucket_hash7_max = CECS_FLATBUCKET8_HASH7_MAX;
static inline cecs_flatset_hash_low_fast cecs_flatset_hash_low_get(const cecs_flatset_hash hash) {
...
static inline const cecs_flatset_hash *cecs_flatbucket_get_hash(const cecs_flatbucket *bucket, const uint_fast8_t index, const size_t value_size, const size_t hash_offset) {
    return ((const cecs_flatset_hash *)(((uint8_t *)cecs_flatbucket_get_value(bucket, index, value_size)) + hash_offset));
}
inline cecs_flatset_hash *cecs_flatbucket_get_hash_mut(cecs_flatbucket *bucket, const uint_fast8_t index, const size_t value_size, const size_t hash_offset) {
    return ((cecs_flatset_hash *)(((uint8_t *)cecs_flatbucket_get_value(bucket, index, value_size)) + hash_offset));
}
...
static void *cecs_flatbucket_insert_expect(
    ...
    const cecs_flatset_hash_low hash7,
    ...
    if (index != bucket_value_count) {
        ...
        void *const value = cecs_flatbucket_push(bucket, value_size);
...
uint_fast8_t cecs_flatbucket_find_hash_index(
    ...
    for (uint_fast8_t i = 0; i < bucket_value_count; i++) {
        const cecs_flatset_hash *stored_hash = cecs_flatbucket_get_hash(bucket, i, hash_stride, hash_offset);
        if (
            (index_mark & (1 << i))
            && (*stored_hash == hash)
        ) {
...
cecs_flatbucket *cecs_flatset_find_insert_bucket_expect(
    ...
    const cecs_flatset_hash_low_fast hash7 = cecs_flatset_hash_low_get(hash);
...
bool cecs_flatset_find(
    ...
    if (cecs_flatset_find_bucket(
        ...
        *out_value = cecs_flatbucket_get_value(bucket, index, value_size);
        return true;
    }
    return false;
}
bool cecs_flatset_find_mut(
    ...
    if (cecs_flatset_find_bucket_mut(
        ...
        *out_value = cecs_flatbucket_get_value_mut(bucket, index, value_size);
        return true;
    }
    return false;
}
...
void *cecs_flatset_insert_into_bucket_expect(
    ...
    void *const value = cecs_flatbucket_insert_expect(bucket, cecs_flatbucket_get_count(*bucket), cecs_flatset_hash_low_get(hash), value_size);
    ++set->values_count;
    return value;
}
...
void *cecs_flatset_find_or_insert(
    ...
    if (index < bucket_value_count) {
        return cecs_flatbucket_get_value_mut(insert, index, value_size);
    } else {
        const cecs_flatset_hash_low_fast hash7 = cecs_flatset_hash_low_get(hash);
