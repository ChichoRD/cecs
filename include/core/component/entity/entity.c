...
world_entities world_entities_create(size_t entity_capacity) {
    ...
    we.entity_ids = cecs_sparse_set_create_of_integers_with_capacity(&we.entity_ids_arena, entity_capacity, sizeof(entity_id));
    we.free_entity_ids = cecs_queue_create_with_capacity(&we.entity_ids_arena, sizeof(entity_id) * entity_capacity);
    return we;
}
...
entity_id world_entities_add_entity(world_entities* we) {
    ...
    if (CECS_QUEUE_COUNT(entity_id, &we->free_entity_ids) > 0) {
        CECS_QUEUE_POP_FIRST(entity_id, &we->free_entity_ids, &we->entity_ids_arena, &id);
...
entity_id world_entities_remove_entity(world_entities* we, entity_id id) {
    entity_id removed;
    if (CECS_SPARSE_SET_REMOVE(entity_id, &we->entity_ids, &we->entity_ids_arena, (size_t)id, &removed)) {
        if (removed != id) {
            assert(false && "unreachable: removed entity_id != id, there is a mismatch in the integer-sparse-set indices");
            exit(EXIT_FAILURE);
        }
        if (CECS_QUEUE_COUNT(entity_id, &we->free_entity_ids) > 0 && id == *CECS_QUEUE_LAST(entity_id, &we->free_entity_ids)) {
            assert(
                false
        ...
        CECS_QUEUE_PUSH_LAST(entity_id, &we->free_entity_ids, &we->entity_ids_arena, &id);
    }
    return id;
}
...
entity_id_range world_entities_add_entity_range(world_entities* we, size_t count) {
    entity_id_range range = {
        .start = world_entities_count(we) + CECS_QUEUE_COUNT(entity_id, &we->free_entity_ids),
        .end = world_entities_count(we) + CECS_QUEUE_COUNT(entity_id, &we->free_entity_ids) + count
    };
