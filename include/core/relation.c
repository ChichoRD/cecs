...
bool world_relations_entity_has_associated_id(world_relations* wr, entity_id entity_id, relation_target target) {
    if (!cecs_sparse_set_contains(&wr->entity_associated_ids, (size_t)entity_id)) {
        ...
        associated_entity_ids* ids = cecs_sparse_set_get_unchecked(&wr->entity_associated_ids, (size_t)entity_id, sizeof(associated_entity_ids));
...
entity_id world_relations_set_associated_id(world_relations* wr, entity_id id, relation_target target, entity_id associated_entity_id) {
    ...
    if (!cecs_sparse_set_contains(&wr->entity_associated_ids, (size_t)id)) {
        associated_entity_ids new_ids = cecs_counted_set_create_with_capacity(
            &wr->entity_associated_ids_arena,
            1,
            sizeof(entity_id)
        );
        ids = CECS_SPARSE_SET_SET(associated_entity_ids, &wr->entity_associated_ids, &wr->entity_associated_ids_arena, (size_t)id, &new_ids);
    }
    else {
        ids = cecs_sparse_set_get_unchecked(&wr->entity_associated_ids, (size_t)id, sizeof(associated_entity_ids));
    }

    return *CECS_COUNTED_SET_SET(entity_id, ids, &wr->entity_associated_ids_arena, (size_t)target.tag_id, &associated_entity_id);
}

entity_id world_relations_get_associated_id(world_relations* wr, entity_id id, relation_target target) {
    associated_entity_ids* ids = cecs_sparse_set_get_unchecked(&wr->entity_associated_ids, (size_t)id, sizeof(associated_entity_ids));
    return *CECS_COUNTED_SET_GET(entity_id, ids, (size_t)target.tag_id);
}
...
entity_id world_relations_increment_associated_id_or_set(world_relations* wr, entity_id id, relation_target target, entity_id otherwise_associated_id) {
    ...
    return *CECS_COUNTED_SET_INCREMENT_OR_SET(entity_id, ids, &wr->entity_associated_ids_arena, (size_t)target.tag_id, &otherwise_associated_id);
}
...
bool world_relations_remove_associated_id(world_relations* wr, entity_id id, relation_target target, entity_id* out_associated_entity_id) {
    if (!cecs_sparse_set_contains(&wr->entity_associated_ids, (size_t)id)) {
        *out_associated_entity_id = 0;
        ...
        associated_entity_ids* ids = cecs_sparse_set_get_unchecked(&wr->entity_associated_ids, (size_t)id, sizeof(associated_entity_ids));
        return cecs_counted_set_remove_out(ids, (size_t)target.tag_id, out_associated_entity_id, sizeof(entity_id));
    }
}
