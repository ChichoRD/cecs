...
dynamic_world_system dynamic_world_system_create_from(components_search_groups s, cecs_arena* a) {
    ...
    for (size_t i = 0; i < s.group_count; ++i) {
        ...
        components_type_info info = CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, group);
        CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, group).component_ids =
            CECS_DYNAMIC_ARRAY_ADD_RANGE(component_id, &d.component_ids, a, info.component_ids, info.component_count);
        CECS_DYNAMIC_ARRAY_ADD(components_search_group, &d.components_search_groups, a, &group);
    }
    return d;
}

components_search_group_range dynamic_world_system_add(dynamic_world_system* d, cecs_arena* a, components_search_group s) {
    components_type_info info = CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, s);
    CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, s).component_ids =
        CECS_DYNAMIC_ARRAY_ADD_RANGE(component_id, &d->component_ids, a, info.component_ids, info.component_count);

    CECS_DYNAMIC_ARRAY_ADD(components_search_group, &d->components_search_groups, a, &s);
    return (components_search_group_range) { 0, CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups) };
}
...
components_search_group_range dynamic_world_system_set(dynamic_world_system* d, cecs_arena* a, components_search_group s, size_t index) {
    assert(index <= CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups) && "index out of bounds");
    if (index == CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups)) {
        dynamic_world_system_add(d, a, s);
        return (components_search_group_range) { 0, index + 1 };
    }

    components_type_info info = CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, s);
    components_type_info previous_info = CECS_UNION_GET_UNCHECKED(
        COMPONENTS_ALL_ID,
        *CECS_DYNAMIC_ARRAY_GET(components_search_group, &d->components_search_groups, index)
    );
    if (info.component_count <= previous_info.component_count) {
        ...
        if (CECS_DYNAMIC_ARRAY_GET(component_id, &d->component_ids, index) != previous_info.component_ids) {
            assert(false && "unreachable: failed component ids index computation");
            exit(EXIT_FAILURE);
        }

        CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, s).component_ids = CECS_DYNAMIC_ARRAY_SET_RANGE(
            component_id,
            &d->component_ids,
        ...
        CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, s).component_ids = CECS_DYNAMIC_ARRAY_ADD_RANGE(
            component_id,
            &d->component_ids,
            a,
            info.component_ids,
    ...
    CECS_DYNAMIC_ARRAY_SET(components_search_group, &d->components_search_groups, index, &s);
    return (components_search_group_range) { 0, index + 1 };
}
...
components_search_group_range dynamic_world_system_set_range(dynamic_world_system* d, cecs_arena* a, components_search_groups s, size_t index) {
    assert(index <= CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups) && "index out of bounds");
    if (index + s.group_count >= CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups) - 1) {
        size_t missing = index + s.group_count - CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups) - 1;
        CECS_DYNAMIC_ARRAY_APPEND_EMPTY(components_search_group, &d->components_search_groups, a, missing);
    }
    ...
    for (size_t i = 0; i < s.group_count; ++i) {
        ...
        if (info.component_count <= previous_info.component_count) {
            ...
            CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, group).component_ids = CECS_DYNAMIC_ARRAY_SET_RANGE(
                component_id,
                &d->component_ids,
                (previous_info.component_ids - (component_id*)d->component_ids.elements),
                info.component_ids,
            ...
            CECS_UNION_GET_UNCHECKED(COMPONENTS_ALL_ID, group).component_ids = CECS_DYNAMIC_ARRAY_ADD_RANGE(
                component_id,
                &d->component_ids,
                a,
                info.component_ids,
        ...
        CECS_DYNAMIC_ARRAY_SET(components_search_group, &d->components_search_groups, index + i, &group);
    }
...
world_system dynamic_world_system_get(const dynamic_world_system* d) {
    return world_system_create(components_search_groups_create(
        ...
        CECS_DYNAMIC_ARRAY_COUNT(components_search_group, &d->components_search_groups)
    ));
}
